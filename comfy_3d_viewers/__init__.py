...
def get_fbx_bundle_path() -> str:
    ...
    return os.path.join(get_three_dir(), "viewer-bundle-three.js")
...
from .viewers import VIEWER_FILES


def _copy_one_file(file_type, filename, subdir, target_dir):
    """Copy a single file (html/bundle/widget) to target directory."""
    import shutil
        ...
        if file_type == "utils" and value is True:
            ...
            for f in src_utils.glob("*.js"):
                ...
                if not dst.exists() or f.stat().st_mtime > dst.stat().st_mtime:
                    shutil.copy2(f, dst)
        elif file_type == "viewer_modules" and value is True:
            src_viewer = Path(get_viewer_dir())
            dst_viewer = target_dir / "js" / "viewer"
            if src_viewer.exists():
                for f in src_viewer.rglob("*"):
                    if f.is_file():
                        rel = f.relative_to(src_viewer)
                        dst = dst_viewer / rel
                        dst.parent.mkdir(parents=True, exist_ok=True)
                        if not dst.exists() or f.stat().st_mtime > dst.stat().st_mtime:
                            shutil.copy2(f, dst)
        elif file_type == "bundles":
            for filename, subdir in value:
                _copy_one_file("bundle", filename, subdir, target_dir)
        elif file_type in ("html", "bundle", "widget"):
            filename, subdir = value
            _copy_one_file(file_type, filename, subdir, target_dir)
