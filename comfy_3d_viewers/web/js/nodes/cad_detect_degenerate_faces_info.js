// Collapsible info box for the CADabra "Detect Degenerate Faces" node: shows
// the per-face report (surface type, bbox center, area, is_valid) returned
// via ui={"text":[...]} after each run. Collapsed by default since the face
// list can get long (100+ faces on real-world assemblies).
import { app } from "../../../scripts/app.js";

const TAG = "[CADDetectDegenerateFacesInfo]";
const COLLAPSED_HEIGHT = 26;
const EXPANDED_HEIGHT = 180;

app.registerExtension({
    name: "cadabra.caddetectdegenerate.info",
    async beforeRegisterNodeDef(nodeType, nodeData) {
        if (nodeData.name !== "CADDetectDegenerateFaces") return;
        ...
        nodeType.prototype.onExecuted = function (message) {
            ...
            const summary = text.split("\n").find((l) => l.includes("Degenerate:")) || "Degenerate faces info";

            let w = this.widgets?.find((x) => x.name === "degenerate_info");
            if (!w) {
                let expanded = false;
                ...
                ta.readOnly = true;
                ta.style.cssText =
                    "width:100%;height:152px;box-sizing:border-box;background:#1a1a1a;color:#ccc;" +
                ...
                w = this.addDOMWidget("degenerate_info", "div", container, {
