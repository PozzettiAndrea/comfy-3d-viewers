// Collapsible info box for the CADabra "CAD Get Filename" node: shows the
// extracted filename(s) returned via ui={"text":[...]} after each run.
// Collapsed by default since batch mode can return a long filename list.
import { app } from "../../../scripts/app.js";

const TAG = "[CADGetFilenameInfo]";
const COLLAPSED_HEIGHT = 26;
const EXPANDED_HEIGHT = 160;

app.registerExtension({
    name: "cadabra.cadgetfilename.info",
    async beforeRegisterNodeDef(nodeType, nodeData) {
        if (nodeData.name !== "CADGetFilename") return;
        ...
        nodeType.prototype.onExecuted = function (message) {
            ...
            const summary = count === 1 ? text : `${count} filename(s)`;

            let w = this.widgets?.find((x) => x.name === "filename_info");
            if (!w) {
                let expanded = false;
                ...
                w = this.addDOMWidget("filename_info", "div", container, {
